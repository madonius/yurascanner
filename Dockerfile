...
RUN apt update\
    && apt upgrade -y\
    && apt install -y libxkbcommon0 libxshmfence1 ca-certificates fonts-liberation\
    ...
    libxdamage1 libxext6 libxfixes3 libxi6 libxrandr2 libxrender1 libxss1 libxtst6 lsb-release wget\
    xdg-utils chromium
ADD . /yurascanner
WORKDIR /yurascanner
RUN mkdir output
